    ...
    match collection {
        Ok(col) => {
            ...
            if !accuracies.is_empty() {
                ...
                let accuracies_data = fs::read_to_string(accuracies_file_path).expect("Unable to read accuracies file");
                let accuracies_json: Vec<f64> = serde_json::from_str(&accuracies_data).expect("Error parsing accuracies JSON");
